...
export default class PieChart extends React.Component {
  ...
  getSectors() {
    ...
    return data.map((d, i) => {
      ...
      const x1 = Math.round(
        center + (center + expandVal) * Math.cos((Math.PI * startAngle) / 180)
      );
      const y1 = Math.round(
        center + (center + expandVal) * Math.sin((Math.PI * startAngle) / 180)
      );

      const x2 = Math.round(
        center + (center + expandVal) * Math.cos((Math.PI * endAngle) / 180)
      );
      const y2 = Math.round(
        center + (center + expandVal) * Math.sin((Math.PI * endAngle) / 180)
      );
  ...
  render() {
    ...
    return (
      <svg
        viewBox={`0 0 ${viewBoxWidth + expandPx * 2} ${viewBoxWidth +
          expandPx * 2}`}
        {...props}
      >
